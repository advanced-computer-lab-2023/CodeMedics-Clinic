...
exports.addFamilyMember = async (req, res) => {
  ...
  try {
    ...
    const exists =
      (await Patient.findOne({ Email: familyMemberEmail })) ||
      (await Patient.findOne({ Number: familyMemberEmail }));
    if (exists == null) {
    ...
    if (me.Linked == patient._id.toString()) {
      return res
        .status(400)
        .json({ message: "You cannot make cyclic relationships" });
    }
...
exports.removeFamilyMemberNoAccount = async (req, res) => {
  ...
  try {
    const me = await Patient.findOne({ Username });
    me.FamilyMembersNoAccount = me.FamilyMembersNoAccount.filter(
...
exports.removeFamilyMember = async (req, res) => {
  ...
  try {
    ...
    if (
      !patient.FamilyMembers.some((el) => el.id.toString() == familyMemberId)
    ) {
      return res.status(400).json({ message: "Family member does not exist" });
    }
